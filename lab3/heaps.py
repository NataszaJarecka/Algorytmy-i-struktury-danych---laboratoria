...
class Heap2:
    ...
    def print_ascii_tree(self, index=0, indent="", is_last=True):
        ...
        children = [(k * index + i) for i in range(1, k + 1)]
        children = [c for c in children if c < len(heap)]

        for c in reversed(children[1:]):
            new_indent = indent + ("│   " if not is_last else "    ")
            self.print_ascii_tree(c, new_indent, False)

        connector = "└── " if is_last else "┌── "
        print(indent + connector + str(heap[index]))

        if children:
            new_indent = indent + ("    " if is_last else "│   ")
            self.print_ascii_tree(children[0], new_indent, True)
